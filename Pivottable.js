            ...
            '<table class="headLeftFixed">' +
                '<thead>' +
                    '{{for headData}}' +
                        '<tr class="head">' +
                            '{{for #data}}' +
                                '<td colspan="{{>cols}}" class="{{if parent}}noOpacity{{/if}}" data-parent="{{>parent}}"  data-value="{{>name}}" rowspan="{{>rows}}" data-id="{{>id}}" data-children="{{>level}}" {{if children}} {{/if}}>{{if name == "total" }}{{else}}{{>name}}{{/if}}</td>' +
                            '{{/for}}' +
                        '</tr>' +
                    '{{/for}}' +
                '</thead>' +
                '<tbody>' +
                    '{{for bodyData}}' +
                        '<tr class="body">' +
                            '{{for #data}}' +
                                '<td colspan="{{>cols}}" class="{{if parent}}bodyLeft{{/if}} noOpacity" data-parent="{{>parent}}" data-value="{{>name}}" data-row="{{>rowkey}}" data-head="{{>headkey}}" rowspan="{{>rows}}" data-children="{{>level}}" data-id="{{>id}}">{{if name == "total" }}{{else}}{{>name}}{{/if}}</td>' +
                            '{{/for}}' +
                        '</tr>' +
                    '{{/for}}' +
                '</tbody>' +
            ...
            '<table class="bodyLeftFixed">' +
                '<thead>' +
                    '{{for headData}}' +
                        '<tr class="head">' +
                            '{{for #data}}' +
                                '<td colspan="{{>cols}}" class="{{if !parent}}invisible {{else}}noOpacity{{/if}}"  data-parent="{{>parent}}"  data-value="{{>name}}" rowspan="{{>rows}}" data-id="{{>id}}" data-children="{{>level}}" {{if children}} {{/if}}>{{if name == "total" }}{{else}}{{>name}}{{/if}}</td>' +
                            '{{/for}}' +
                        '</tr>' +
                    '{{/for}}' +
                '</thead>' +
                '<tbody>' +
                    '{{for bodyData}}' +
                        '<tr class="body">' +
                            '{{for #data}}' +
                                '<td colspan="{{>cols}}" class="{{if parent}}bodyLeft {{else}}noOpacity{{/if}}" data-parent="{{>parent}}" data-value="{{>name}}" data-row="{{>rowkey}}" data-head="{{>headkey}}" rowspan="{{>rows}}" data-children="{{>level}}" data-id="{{>id}}">{{if name == "total" }}{{else}}{{>name}}{{/if}}</td>' +
                            '{{/for}}' +
                        '</tr>' +
                    '{{/for}}' +
                '</tbody>' +
...
Pivottable.prototype = $.extend(new E_Base(), {
    init: function(cfg) {
        cfg = cfg || {};
    ...
    */  
    ...
    bindEvent: function() { 
        ...
        this.wrap.find('.transverseRollingWrap').off().on('scroll', function(){
            self.wrap.find('.bodyLeftFixed').css('margin-top', -$(this).scrollTop() + 'px');
            self.wrap.find('.headRightFixed').css('margin-left', -$(this).scrollLeft() + 'px');
        })
    ...
    resetStyle: function(){
        ...
        var height = $defaultTable[0].offsetHeight + self.scrollWidth < self.height ? $defaultTable[0].offsetHeight + self.scrollWidth : self.height;
        var width = $defaultTable[0].offsetWidth + self.scrollWidth < self.width ? $defaultTable[0].offsetWidth + self.scrollWidth : self.width;
    ...
    render: function() {
        ...
        if(!this.width){
            this.width = this.wrap[0].offsetWidth;
